...
###
def QFSD(use_hd=True):
    datasets = {}
    folders_path = "/home/iim/eee/data/"
    images_path = os.path.join(folders_path, 'QianFSD')
    list_files = os.listdir(images_path)
    num_elements = {}
    buffer = {'train': 0, 'val': 98, 'test': 118}
    class_names = {}
    for subset in ['train', 'val', 'test']:
        data = []
        target = []
        ...
        csv_path = os.path.join(folders_path, f'{subset}.csv')
        class_names[subset] = []
        with open(csv_path, "r") as f:
            start = 0
            for line in f:
                if start == 0:
                    ...
                    splits = line.split(",")
                    fn, c = splits[0], splits[1].replace('\n', '')
                    class_names[subset].append(c)
        files = list(set([fn for fn in class_names[subset]]))
        for c, folder in enumerate(files):
            count = 0
            images = os.listdir(os.path.join(images_path, folder))
            for file in images:
    ...
    98, 20, 25, (num_elements['train'], num_elements['val'], num_elements['test'])), True, False
